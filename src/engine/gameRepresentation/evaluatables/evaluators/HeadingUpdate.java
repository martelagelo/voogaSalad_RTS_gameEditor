...
public class HeadingUpdate<A, B> extends Evaluator<A, B, Boolean> {
	...
	public HeadingUpdate(String id, Evaluatable<A> parameter1,
	...
	public Boolean evaluate(GameElement element1, GameElement element2) {
		...
		double tempXGoal = element.getNumericalAttribute(
				StateTags.X_TEMP_GOAL_POSITION.getValue()).doubleValue();
		double tempYGoal = element.getNumericalAttribute(
				StateTags.Y_TEMP_GOAL_POSITION.getValue()).doubleValue();
		double xGoal = element.getNumericalAttribute(
				StateTags.X_GOAL_POSITION.getValue()).doubleValue();
		double yGoal = element.getNumericalAttribute(
				StateTags.Y_GOAL_POSITION.getValue()).doubleValue();
		double speed = element.getNumericalAttribute(
