...
public class VisionComputer extends Computer<SelectableGameElement, DrawableGameElement> {
    ...
    protected boolean checkComputingCondition (SelectableGameElement primaryObject,
        ...
        if (primaryObject instanceof Sighted && otherObject instanceof Boundable) {
            ...
            return new Polygon(sightedObject.getVisionBounds()).intersects(new Polygon(
                    boundableObject.getBounds()).getBoundsInLocal());
        } else {
            return false;
        }
    }
    ...
    /**
     * Add the elements of interest to the primary object's visible elements
     * list
     * 
     * @see Computer#givePrimaryObjectElements
     */
    @Override
    protected void givePrimaryObjectElements (SelectableGameElement primaryObject,
