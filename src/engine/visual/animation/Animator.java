...
public class Animator implements Updatable {
	...
	private void determineAnimationDirection() {
		double xVelocity = attributesOfInterest.getNumericalAttribute(StateTags.X_VELOCITY).doubleValue();
		double yVelocity = attributesOfInterest.getNumericalAttribute(StateTags.Y_VELOCITY).doubleValue();
	...
	private Rectangle2D getViewport(int frameNumber) {
		...
		return new Rectangle2D(colNumber * myState.getViewportSize().getWidth(), rowNumber
				* myState.getViewportSize().getHeight(), myState.getViewportSize().getWidth(), myState
