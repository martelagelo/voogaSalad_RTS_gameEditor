...
/**
 * An evaluator that implements classical game bouncing. element 1 bounces off
 * of element 2
 *
 * @author Zach
 *
...
public class Bounce<A, B> extends Evaluator<A, B, Boolean> {
    ...
    public Bounce (Evaluatable<A> parameter1, Evaluatable<B> parameter2) {
        super(Boolean.class, "bounce", parameter1, parameter2);
    }
    ...
    protected Boolean evaluate (GameElement element1, GameElement element2) {
        ...
        element1.setNumericalAttribute(StateTags.X_VELOCITY.getValue(),
                Math.round(speed * Math.cos(angle)));
        element1.setNumericalAttribute(StateTags.Y_VELOCITY.getValue(),
                Math.round(speed * Math.sin(angle)));
        return true;

    }

    /**
     * Get the center of one element
     *
     * @param element
     * @return a Point2D of the element center
     */
    private Point2D getCenter (GameElement element) {
        Point2D center = new Point2D();
        center.setLocation(element.getNumericalAttribute(StateTags.X_POSITION.getValue())
                .floatValue(), element.getNumericalAttribute(StateTags.Y_POSITION.getValue())
                .floatValue());
        return center;
    }
