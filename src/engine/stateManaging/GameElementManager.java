...
public class GameElementManager {
	...
	public List<GameElement> findAllElementsOfType(String typeName) {
		return myLevel.getUnits().stream().filter(o -> o.isType(typeName))
	...
	public SelectableGameElement addSelectableGameElementToLevel(
			String typeName, double x, double y, String color) {
	 ...
	 */
	public void selectUnitsInBounds(double[] rectPoints, boolean multiSelect,
			Participant u) {
	...
	public void selectSingleUnit(Point2D clickLoc, boolean multiSelect,
			Participant u) {
		for (SelectableGameElement e : myLevel.getUnits()) {
	...
	public void setSelectedUnitCommand(Point2D click, boolean queueCommand,
			Participant u) {
		// check to see if a unit was right-clicked on
		if (filterTeamIDElements(filterSelectedUnits(myLevel.getUnits()), u)
				.size() > 0) {
		...
		for (SelectableGameElement e : filterSelectedUnits(myLevel.getUnits())) {
			if (!checkOnTeam(e, u))
				...
				if (queueCommand) {
					...
					double currentX = e.getNumericalAttribute(
							StateTags.X_POSITION).doubleValue();
					double currentY = e.getNumericalAttribute(
					...
					List<Location> waypoints = pathingComputer.findPath(from,
							to);
	...
	private List<SelectableGameElement> filterSelectedUnits(
			List<SelectableGameElement> list) {
		return list
				.stream()
				.filter(unit -> unit.getNumericalAttribute(
						StateTags.IS_SELECTED).doubleValue() == 1)
				.collect(Collectors.toList());
	}

	private List<SelectableGameElement> filterTeamIDElements(
			List<SelectableGameElement> list, Participant p) {
		return list
				.stream()
				.filter(e -> p.checkSameTeam(e
						.getTextualAttribute(StateTags.TEAM_COLOR)))
				.collect(Collectors.toList());
	}
