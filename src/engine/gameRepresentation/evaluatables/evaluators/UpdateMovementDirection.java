...
public class UpdateMovementDirection<A, B> extends Evaluator<A, B, Boolean> {
    ...
    public Boolean evaluate (GameElement element1, GameElement element2) {
        ...
        double deltaX =
                element1.getNumericalAttribute(StateTags.X_TEMP_GOAL_POSITION.getValue()).doubleValue() -
                        element1.getNumericalAttribute(StateTags.X_POSITION.getValue()).doubleValue();
        double deltaY =
                element1.getNumericalAttribute(StateTags.Y_TEMP_GOAL_POSITION.getValue()).doubleValue() -
        ...
        if (deltaX != 0 || deltaY != 0) {
            element1.setNumericalAttribute(StateTags.X_VELOCITY.getValue(), Math.round(speed * Math.cos(direction)));
            element1.setNumericalAttribute(StateTags.Y_VELOCITY.getValue(), Math.round(speed * Math.sin(direction)));
        }
        else{
