...
public class ElementAccordionController extends GUIContainer {
	...
	private void updateList(ElementDropDownController dropDownController, List<ImageElementPair> units) {
		units.forEach((item) -> {
	...
	private Consumer<Consumer<WizardData>> openSelectableGameElementWizard() {
		Consumer<Consumer<WizardData>> consumer = (c) -> {
			...
			Consumer<WizardData> cons = (data) -> {
				Optional<SelectableGameElementState> sameElementExistsOption = myMainModel.getGameUniverse()
						.getSelectableGameElementStates().stream()
