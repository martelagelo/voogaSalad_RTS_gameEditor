...
=======
	...
	public DescribableState getDescribableState(String[] selection) throws CampaignNotFoundException,
			LevelNotFoundException {
		if (selection[2].isEmpty()) {
			if (selection[1].isEmpty()) {
				return myGameState;
	...
	public void setLevel(LevelIdentifier levelID, LevelState levelState) throws CampaignNotFoundException,
			LevelNotFoundException {
	 ...
	 * @throws LevelExistsException
	 */
	public void createLevel(String levelName, String campaignName, Number width, Number height, String backgroundPath)
			throws LevelExistsException, CampaignNotFoundException, Exception {
		CampaignState campaignState = myGameState.getCampaign(campaignName.trim());
		LevelState newLevelState = new LevelState(levelName.trim(), campaignName.trim());
		newLevelState.attributes.setTextualAttribute(StateTags.BACKGROUND_PATH.name(), backgroundPath);
		if (width.doubleValue() > 0 && height.doubleValue() > 0) {
			newLevelState.attributes.setNumericalAttribute(StateTags.LEVEL_WIDTH.getValue(), width);
	...
	public void createDrawableGameElementState(WizardData data) {
		try {
			String actualSaveLocation = mySaveLoadMediator.saveImage(data, GameElementImageType.Drawable);
			String colorMaskLocation = mySaveLoadMediator.saveColorMask(data, GameElementImageType.Drawable);
			data.addDataPair(WizardDataType.IMAGE, actualSaveLocation);
			data.addDataPair(WizardDataType.COLOR_MASK, colorMaskLocation);
			DrawableGameElementState gameElement = GameElementStateFactory
					.createDrawableGameElementState(data);
	...
	public void createSelectableGameElementState(WizardData data) {
		try {
			String actualSaveLocation = mySaveLoadMediator.saveImage(data, GameElementImageType.Selectable);
			String colorMaskLocation = mySaveLoadMediator
					.saveColorMask(data, GameElementImageType.Selectable);
			data.addDataPair(WizardDataType.IMAGE, actualSaveLocation);
			data.addDataPair(WizardDataType.COLOR_MASK, colorMaskLocation);
	...
	public void removeDrawableGameElement(String elementName) throws ElementInUseException {
		if (elementIsInUse(elementName))
			throw new ElementInUseException(elementName);
		Optional<DrawableGameElementState> option = getGameUniverse().getDrawableGameElementStates().stream()
				.filter((state) -> {
					return state.getName().equals(elementName);
				}).findFirst();
		if (option.isPresent()) {
	...
	public void removeSelectableGameElement(String elementName) throws ElementInUseException {
		if (elementIsInUse(elementName))
			throw new ElementInUseException(elementName);
		Optional<SelectableGameElementState> option = getGameUniverse().getSelectableGameElementStates()
				.stream().filter((state) -> {
					return state.getName().equals(elementName);
				}).findFirst();
		if (option.isPresent()) {
	...
	public SpriteImageContainer fetchImageContainer(String imageTag) {
		...
		ImageView newImageView = new ImageView(matchingContainer.getSpritesheet().getImage());
		ImageView newColorMaskImageView = new ImageView(matchingContainer.getColorMask().getImage());
		SpriteImageContainer spriteContainer = new SpriteImageContainer(newImageView, newColorMaskImageView);
		return spriteContainer;
	}
>>>>>>> Stashed changes
}
