...
/**
 * Game Runner View Controller holds scene graph as well as button pane, status
 * pane, and minimap
 * 
...
public class GameRunnerPaneController extends StackPaneGUIContainer {
	...
	public LevelState getLevelState() {
        System.out.println("Model version of level state after unit addition: " + myLevel.toJSON());
		return myLevel;
	}
	...
	public void setLevel(LevelState levelState) {
		try {
			...
			// require button to fill actual size of borderpane to then be bound
			// to runner pane size
	...
	public void engineUpdate(AttributeContainer attributes) {
		List<String> attributesToShow = attributes.getNumericalAttributes().stream()
				.map((attribute) -> String.format("%s: %s", attribute.getName(), attribute.getData()))
