...
/**
 * Factory that takes in GameElementStates and generates their JavaFX
 * representations as well as their Evaluatables, then packs it all into the
 * rendered wrapper used in the engine.
 *
...
public class GameElementFactory {
    ...
    private EvaluatableFactory myActionFactory;
    private VisualizerFactory myVisualizerFactory;

    public GameElementFactory (GameUniverse universe, EvaluatableFactory actionFactory,
    ...
    public DrawableGameElement createDrawableGameElement (DrawableGameElementState state) {
        DrawableGameElement element = new DrawableGameElement(state, generateVisualizer(state));
        generateActions(element, state);
        return element;
    }

    public SelectableGameElement createSelectableGameElement (String elementType, double x,
            double y, Number color) {
        SelectableGameElementState state = myUniverse.getSelectableGameElementState(elementType);
        state.myAttributes.setNumericalAttribute(StateTags.TEAM_COLOR.getValue(), color);
    ...
    /**
     * Iterate through the actions within an element's state and instantiate all
     * the element's actions
     *
     * @param element
     *            the game element
     * @param state
     *            the game element's state
     */
