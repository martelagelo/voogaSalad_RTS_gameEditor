...
public class ShittyMain extends Application {
	...
	private Engine hardCodeAGame() throws Exception {
		SelectableGameElementState minerState = createMiner(new double[] { 0,
				0, 40, 0, 40, 40, 0, 40 }, 10, 100, 1);
		minerState.attributes.setTextualAttribute(
		...
		SelectableGameElementState resource = createResource(new double[] { 0,
				0, 40, 0, 40, 40, 0, 40 }, 300, 300, 1);
		resource.attributes.setTextualAttribute(StateTags.NAME.getValue(),
				"resource1");
		...
		minerState.addAction(new ActionWrapper(ActionType.COLLISION,
				ActionOptions.INCREMENT_DECREMENT_ACTION, "gold", "ORE",
				"goodness", "0", "resources", "MiningTimer", "100"));
		...
		archerState.attributes.setTextualAttribute(
				StateTags.ATTRIBUTE_DESCRIPTION.getValue() + 1,
				"resources/img/graphics/buttons/buildings/0001.bmp");

		SelectableGameElementState archerState1 = createArcher(new double[] {
				0, 0, 40, 0, 40, 40, 0, 40 }, 200, 400, 1);
		...
		SelectableGameElementState archerState2 = createArcher(new double[] {
				0, 0, 40, 0, 40, 40, 0, 40 }, 400, 100, 2);
		archerState2.attributes.setTextualAttribute(
				StateTags.TEAM_COLOR.getValue(), "RED");
		archerState2.attributes.setNumericalAttribute(
				StateTags.HEALTH.getValue(), 100);
		archerState2.attributes.setNumericalAttribute(
				StateTags.MOVEMENT_SPEED.getValue(), 0);
		...
		archerState2.addAction(new ActionWrapper(ActionType.INTERNAL,
				ActionOptions.ATTRIBUTE_INCRIMENT_ACTION, StateTags.HEALTH
						.getValue(), StateTags.ATTACK.getValue(), "MAX_HEALTH",
				"50", "Random Timer"));

		SelectableGameElementState archerState3 =

		createArcher(new double[] { 0, 0, 40, 0, 40, 40, 0, 40 }, 400, 300, 2);
		archerState3.attributes.setTextualAttribute(
				StateTags.TEAM_COLOR.getValue(), "GREEN");

		// Make the third archer spawn archers on collision
		archerState3.attributes.setNumericalAttribute(
				StateTags.X_SPAWN_OFFSET.getValue(), 500);
		archerState3.attributes.setNumericalAttribute(
				StateTags.Y_SPAWN_OFFSET.getValue(), 500);
		...
		levelState.attributes.setNumericalAttribute(
				StateTags.LEVEL_WIDTH.getValue(), 2000);
		levelState.attributes.setNumericalAttribute(
				StateTags.LEVEL_HEIGHT.getValue(), 2000);
		...
		Engine engine = new Engine(model2, model2.getLevel("testCampaign",
				"testLevel"));
	...
	private GameElementState createGoal() {
		...
		ges.addAction(new ActionWrapper(ActionType.INTERNAL,
				ActionOptions.PLAYER_ATTRIBUTE_CONDITION, "my", "Resources",
				"GreaterThanEqual", "1000", "Won", "EqualsAssignment", "1"));
		ges.addAction(new ActionWrapper(ActionType.INTERNAL,
				ActionOptions.OBJECT_LOCATION_DETECTION, "my", "archer", "50",
				"50", "50", "Won", "EqualsAssignment", "1"));
		return ges;
	}

	private SelectableGameElementState createResource(double[] bounds,
			double x, double y, int teamID) throws Exception {
		SelectableGameElementState archerState = new SelectableGameElementState(
				x, y);
		// archerState.attributes.setNumericalAttribute(StateTags.TEAM_ID,
		// teamID);
		archerState.attributes.setNumericalAttribute(
				StateTags.X_POSITION.getValue(), x);
		archerState.attributes.setNumericalAttribute(
				StateTags.Y_POSITION.getValue(), y);
		archerState.attributes.setNumericalAttribute(
				StateTags.RELOAD_TIME.getValue(), 50);
		archerState.attributes.setTextualAttribute(
		...
		AnimatorState archerAnimations = SaveLoadUtility.loadResource(
				AnimatorState.class,
		...
		SelectableGameElementState sges = SaveLoadUtility.loadResource(
				SelectableGameElementState.class, "resources/sges.json");
		Map<ActionType, List<ActionWrapper>> map = sges.getActions();

		return archerState;
	}

	private SelectableGameElementState createMiner(double[] bounds, double x,
			double y, int teamID) throws Exception {
		SelectableGameElementState minerState = new SelectableGameElementState(
				x, y);
		// minerState.attributes.setNumericalAttribute(StateTags.TEAM_ID,
		// teamID);
		minerState.attributes.setNumericalAttribute(
				StateTags.X_POSITION.getValue(), x);
		minerState.attributes.setNumericalAttribute(
				StateTags.Y_POSITION.getValue(), y);
		minerState.attributes.setNumericalAttribute(
		...
		minerState.attributes.setNumericalAttribute(
				StateTags.HEALTH.getValue(), 500);
		minerState.attributes.setNumericalAttribute(
				StateTags.ATTACK.getValue(), 75);
		minerState.attributes.setNumericalAttribute(
				StateTags.RELOAD_TIME.getValue(), 50);
		minerState.attributes.setTextualAttribute(
				StateTags.CURRENT_ACTION.getValue(), "STANDING");
		...
		minerState.addAction(new ActionWrapper(ActionType.COLLISION,
				ActionOptions.OBJECT_CONDITION_ACTION, "NotCollision",
				"RandomWaypoint"));
		// Move back if we collide with anything
		minerState
				.addAction(new ActionWrapper(ActionType.COLLISION,
						ActionOptions.OBJECT_CONDITION_ACTION, "Collision",
						"MoveBack"));
		// Check to see if our health is <0. If so, die.
		minerState.addAction(new ActionWrapper(ActionType.INTERNAL,
				ActionOptions.CHECK_ATTR_SET_ATTR_ACTION, StateTags.HEALTH
						.getValue(), "LessThanEqual", "0", StateTags.IS_DEAD
						.getValue(), "EqualsAssignment", "1"));
	...
	private SelectableGameElementState createArcher(double[] bounds, double x,
			double y, int teamID) throws Exception {
		SelectableGameElementState archerState = new SelectableGameElementState(
				x, y);
		archerState.attributes.setNumericalAttribute(
		...
		archerState.attributes.setNumericalAttribute(
				StateTags.X_SPAWN_OFFSET.getValue(), 100);
		archerState.attributes.setNumericalAttribute(
				StateTags.Y_SPAWN_OFFSET.getValue(), 100);
