...
public class GameElementManager implements Observer {
	...
	private void selectPlayerUnits(double[] rectPoints) {
		for (SelectableGameElement e : myLevel.getUnits()) {
			...
			polygonBounds.getPoints().addAll(
					new Double[] { bounds[0], bounds[1], bounds[0] + bounds[2],
							bounds[1], bounds[0] + bounds[2],
							bounds[1] + bounds[3], bounds[0],
							bounds[1] + bounds[3] });

			if (polygonBounds.intersects(rectPoints[0], rectPoints[1],
					rectPoints[2] - rectPoints[0], rectPoints[3]
							- rectPoints[1])) {
				e.select(true);
	...
	public void update(Observable o, Object arg) {
		if (o instanceof SelectionBox) {
			...
			System.out
					.println("Click: "
							+ (((ClickManager) o).isPrimary() ? "primary"
									: "secondary") + ", loc: " + click.getX()
							+ ", " + click.getY());
